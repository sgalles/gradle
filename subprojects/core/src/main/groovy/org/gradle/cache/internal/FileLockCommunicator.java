...
public class FileLockCommunicator {
    ...
    public static void pingOwner(String ownerPort, File target) {
        try {
            DatagramSocket datagramSocket = new DatagramSocket();
            byte[] bytesToSend = encodeFile(target);
            datagramSocket.send(new DatagramPacket(bytesToSend, bytesToSend.length, InetAddress.getLocalHost(), Integer.parseInt(ownerPort)));
        } catch (Exception e) {
